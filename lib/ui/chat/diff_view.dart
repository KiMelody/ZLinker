...
class DiffData {
  ...
  /// Raw line counts (+/-) for the summary chips; null when unavailable.
  final int additions;
  final int deletions;
                ...
                for (final line in diff.lines.take(400))
                  Container(
                    color: switch (line.type) {
                      DiffLineType.added =>
                        ZColors.success.withValues(alpha: 0.12),
                      DiffLineType.removed =>
                        ZColors.danger.withValues(alpha: 0.12),
