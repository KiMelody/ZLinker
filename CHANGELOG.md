...
- **Android release signing**: CI-built APKs are now signed with a dedicated
  release keystore (GitHub Secrets + `key.properties`), replacing the debug
  key. Existing debug-signed installs must be uninstalled before updating.
